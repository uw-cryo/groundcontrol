...
def test_raster_footprint_is_valid_data_only(tmp_path):
    ...
    fp = aoi_mod.raster_footprint(dem, valid=True)
    ...
    assert aoi_mod.raster_footprint(full, valid=True).to_crs(CRS).geometry.iloc[0].area \
...
def test_raster_footprint_decimated_read_matches_full(tmp_path):
    ...
    coarse = aoi_mod.raster_footprint(dem, max_px=50, valid=True).to_crs(CRS).geometry.iloc[0]
    fine = aoi_mod.raster_footprint(dem, max_px=200, valid=True).to_crs(CRS).geometry.iloc[0]
...
def test_raster_footprint_refuses_no_crs_and_no_data(tmp_path):
    ...
    with pytest.raises(ValueError, match="no CRS"):
        aoi_mod.raster_footprint(p, valid=True)
    ...
    with pytest.raises(ValueError, match="no valid pixels"):
        aoi_mod.raster_footprint(p2, valid=True)
...
def test_assess_explicit_aoi_raster_and_site_name_still_win(tmp_path, monkeypatch):
    ...
    assert seen["aoi"].geometry.iloc[0].equals(
        aoi_mod.raster_footprint(other, valid=True).geometry.iloc[0])
...
def test_raster_footprint_rotated_and_untagged_nan(tmp_path):
    ...
    for deg in (30.0, 90.0, 180.0):
        ...
        fp = aoi_mod.raster_footprint(p, max_px=20, valid=True).to_crs(CRS).geometry.iloc[0]
    ...
    with rasterio.open(p, "w", driver="GTiff", height=n, width=n, count=1, dtype="float32",
        ...
        d.write(arr, 1)
    assert aoi_mod.raster_footprint(p, valid=True).to_crs(CRS).geometry.iloc[0].area \
...
def test_raster_footprint_small_patch_survives_decimation(tmp_path):
    ...
    with rasterio.open(p, "w", driver="GTiff", height=n, width=n, count=1, dtype="float32",
                       ...
                       transform=from_origin(400000.0, 3650000.0, 30.0, 30.0)) as d:
        d.write(arr, 1)
    fp = aoi_mod.raster_footprint(p, max_px=8, valid=True).to_crs(CRS).geometry.iloc[0]
    assert fp.area > 0 and fp.covers(aoi_mod.raster_footprint(p, max_px=256, valid=True)
    ...
    with pytest.raises(ValueError, match="no valid pixels at 1/32 decimation"):
        aoi_mod.raster_footprint(p, max_px=8, valid=True)
...
def test_raster_footprint_warns_when_mask_comes_from_overviews(tmp_path, caplog):
    ...
    with caplog.at_level("WARNING", logger="groundcontrol.aoi"):
        fp = aoi_mod.raster_footprint(p, max_px=32, valid=True)
...
def test_raster_footprint_caps_fragmented_mosaics(tmp_path, monkeypatch, caplog):
    ...
    with caplog.at_level("WARNING", logger="groundcontrol.aoi"):
        gdf = A.raster_footprint(path, valid=True)
    ...
    exact = A.raster_footprint(path, valid=True).to_crs("EPSG:32612").geometry.iloc[0]
...
def test_raster_footprint_default_is_grid_extent(tmp_path):
    ...
    exact = aoi_mod.raster_footprint(dem, valid=True)
    import rasterio
    from shapely.geometry import box
    with rasterio.open(dem) as src:
